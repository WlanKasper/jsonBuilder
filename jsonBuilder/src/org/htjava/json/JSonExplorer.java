...
public class JSonExplorer {
	...
	public static Integer getObjValueInt(JSonObj jsObj,String name) {
		...
		if (elRes==null)
			return null;
		if (elRes instanceof JSonInteger)
			return ((JSonInteger)elRes).value();
		if (elRes instanceof JSonNumber)
			return ((JSonNumber)elRes).toIntValue();
		throw new RuntimeException("unexpected type "+elRes.getClass().getSimpleName());
	}
	...
	public static Long getObjValueLong(JSonObj jsObj,String name) {
		...
		if (elRes==null)
			return null;
		if (elRes instanceof JSonLong)
			return ((JSonLong)elRes).value();
		if (elRes instanceof JSonInteger)
			return (long)((JSonInteger)elRes).value();
		if (elRes instanceof JSonNumber)
			return ((JSonNumber)elRes).toLongValue();
		throw new RuntimeException("unexpected type "+elRes.getClass().getSimpleName());
	}
	...
	public static void inspect(JSonElem elem, Consumer<JSonElem> elemCons, Consumer<NameValue> nvCons, boolean includeValues) {
		inspectinner(elem, elemCons, nvCons, includeValues);
	}

	private static void inspectinner(JSonElem elem, Consumer<JSonElem> elemCons, Consumer<NameValue> nvCons, boolean includeValues) {
	...
	private static void inspectinner(NameValue nv, Consumer<JSonElem> elemCons, Consumer<NameValue> nvCons, boolean includeValues) {
		if (nvCons!=null) nvCons.accept(nv);
		inspectinner(nv.getValue(), elemCons, nvCons,includeValues);
	}
	/* ----------- INSPECT FINE  -------------- */
